...
**균열은 애니메이션이 아니라 워드마크의 형태입니다.** 띠를 파편 20조각으로
자르고 각 조각을 무게중심 쪽으로 당겨서, 가만히 있어도 이음새마다 배경이
비치는 균열선이 남습니다. 등장할 때는 이미 보이던 그 선을 따라 깨졌다가
제자리로 맞물립니다.

- **조각 수는 성능이 아니라 디자인 결정입니다.** 촘촘하면 모자이크나 굴절로
  읽히고, 성기어야 "깨진 것"으로 읽힙니다. 20조각이면 한 조각이 글자 하나를
  통째로 물 정도의 크기입니다.
- 균열 폭은 `CRACK`(1.6, 실제 간격은 그 2배)입니다. 1440 기준 약 3px이고,
  잉크의 1.2% 정도가 균열로 빠집니다. **부호를 뒤집으면 균열이 닫히고 그냥
  통짜 글자로 돌아갑니다** — 이 부호가 디자인 전부입니다.
- 격자 꼭짓점은 결정론적으로 흔들되(`rnd`) **바깥 테두리는 흔들지 않습니다.**
  테두리가 어긋나면 타일링이 깨져서 균열이 아니라 구멍이 됩니다.
- 조각이 크면 이동 폭도 줄여야 합니다. 작은 조각용 진폭을 그대로 쓰면 마크가
  맞춰지는 게 아니라 파편이 흩날리는 걸로 보입니다.
